...
public class RoadModule extends ConfigurableWorldModule {
	...
	private static final LaneType SOLID_LINE = new FlatTexturedLane(
		...
		public Double getAbsoluteWidth(TagGroup roadTags, TagGroup laneTags) {
			double width;
			// System.out.println("physical lane tag: " + laneTags); 
			if (laneTags.containsKey("divider")){
				width = parseMeasure(laneTags.getValue("divider"));
				System.out.println("Sdivider width: " + width);
				return width;
			}
	...
	//IMPORTANT: ADDED PHYSICAL DIVIDER
	private static final LaneType PHYSICAL_LINE = new FlatTexturedLane(
		"PHYSICAL_LINE", false, false) {
		...
		public Double getAbsoluteWidth(TagGroup roadTags, TagGroup laneTags) {
			...
			if (laneTags.containsKey("divider")){
				width = parseMeasure(laneTags.getValue("divider"));
				System.out.println("physical width: " + width);
				return width;
			}
			width = parseWidth(laneTags, 1.0f);
			// System.out.println("physical width: " + width);  
			return width;
		}
		...
		@Override
		protected Material getSurface(TagGroup roadTags, TagGroup laneTags) {
			return ROAD_MARKING_DIVIDER;
		}
	
	};

	//IMPORTANT: ADDED PHYSICAL DIVIDER
	private static final LaneType UTURN_LINE = new FlatTexturedLane(
		"UTURN_LINE", false, false) {

		@Override
		public Double getAbsoluteWidth(TagGroup roadTags, TagGroup laneTags) {
			double width;
			System.out.println("UTURN lane tag: " + laneTags);  
			if (laneTags.containsKey("divider")){
				width = parseMeasure(laneTags.getValue("divider"));
				System.out.println("uturn width: " + width);
				return width;
			}
			width = parseWidth(laneTags, 1.0f);
			// System.out.println("physical width: " + width);  
